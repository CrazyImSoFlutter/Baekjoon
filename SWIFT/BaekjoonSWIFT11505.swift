...
func multiply(_ node: Int, _ start: Int, _ end: Int, _ left: Int, _ right: Int) -> Int {
    ...
    let mid = (start + end) / 2
    let leftResult = multiply(node * 2, start, mid, left, right)
    let rightResult = multiply(node * 2 + 1, mid + 1, end , left, right)
...
for i in 0..<C.count {
    ...
    if cmd == 1 {
        ...
        print(multiply(1, 0, N - 1, idx, idx2) % MOD)
    }
}
